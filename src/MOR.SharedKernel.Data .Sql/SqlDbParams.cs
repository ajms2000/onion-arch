...
namespace System.Data.Sql
{
    public class SqlDbParams : IDbParams
    {
        ...
        private List<DbParameter> GenerateParametersCore()
        {
            var ret = new List<DbParameter>();

            foreach (var kvp in SPPItems)
            {
                var sp = default(DbParameter);
                var spp = kvp.Value;
                ...
                {
                    sp = new Microsoft.Data.SqlClient.SqlParameter(field, string.Empty);
                ...
                {
                    sp = new Microsoft.Data.SqlClient.SqlParameter(field, spp.ValueObject ?? DBNull.Value);
                    sp.DbType = spp.DataType;
